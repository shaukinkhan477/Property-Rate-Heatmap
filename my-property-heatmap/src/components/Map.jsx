...
const filteredProperties = (selectedLocationProperties.length > 0 ? selectedLocationProperties : selectedCityProperties).filter(
  ...
  const handleMarkerClick = (city) => {
    ...
    if (cityLocations) {
      setSelectedCityProperties(cityLocations.locations.flatMap(loc => loc.properties || []));
    ...
    <div style={{ display: 'flex' }}>     
      ...
      <MapContainer center={mapCenter} zoom={zoomLevel} style={{ height: '700px', width: '70%' }} zoomControl={false}>
        <LayersControl position="topright">
          ...
          <LayersControl.BaseLayer name="3D View">
            <TileLayer
        ...
        {Object.keys(cityCoordinates).map((city, index) => (
          <Marker key={index} position={cityCoordinates[city]} icon={icon} eventHandlers={{ click: () => handleMarkerClick(city) }}>
          ...
          >
            ...
            <Popup>
              ...
              <div className="price-row">
                ...
                <span className="price-highlight">₹{location.prices.year1} /Sq.ft</span>
              </div>
              <div className="price-row">
                <span className="year">2 Years Ago:</span>
                <span className="price-highlight">₹{location.prices.year2} /Sq.ft</span>
              </div>
              <div className="price-row">
                <span className="year">1 Year Ago:</span>
                <span className="price-highlight">₹{location.prices.year3} /Sq.ft</span>
              </div>
              <div className="price-row">
